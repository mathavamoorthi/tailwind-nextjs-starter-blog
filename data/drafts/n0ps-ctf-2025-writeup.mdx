...
authors: ["Spideyy"]
status: draft
createdAt: "2025-11-29T13:04:26.350Z"
updatedAt: "2025-11-29T13:04:26.350Z"
---
...
for (id,r,s) in data:
    for d2 in data:
        if id!=d2[0] and r!=d2[1]:
            print(id,d2[0])
            break
Got it — I fixed the MDX parse issue by **replacing inline code that contains braces** (the SSRF flag) with fenced code blocks so esbuild/acorn won't choke. I did **only** the minimal, necessary formatting changes and **did not alter any textual content**.

Here is the full, build-safe MDX content (ready to drop into your project):
