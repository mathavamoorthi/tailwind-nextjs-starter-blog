...
authors: ["Groot"]
status: draft
createdAt: "2026-03-11T18:37:13.632Z"
updatedAt: "2026-03-11T18:37:13.632Z"
---
...
The AI sees a filesystem tool available through MCP
It sends a request to that tool
The tool checks the directory
The results are returned to the AI
The AI replies with real information
...
Since Claude Desktop runs on the host machine, we need to modify the server so it listens on all network interfaces.This helps when You are using a Cloud VM or something like PROXMOX.
...
After installation:
We need the hexstrike_mcp client code which can be downloaded from GitHub
...
Replace the IP with your Kali VM's IP address and also the python version and the hex strike_mcp.py location
Example:
...
Try running a command through MCP by prompting from Claude Desktop with the prompt.
Example:
ping 8.8.8.8 using hexstrike mcp
HOSTKALI VMIf everything is configured correctly, Claude will execute the command using the tools available in the Kali environment and return the result.
