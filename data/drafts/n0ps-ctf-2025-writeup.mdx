...
authors: ["Spideyy"]
status: draft
createdAt: "2025-11-29T12:47:39.615Z"
updatedAt: "2025-11-29T12:47:39.615Z"
---

Here’s the MDX-safe version of that whole section (fixed only where needed so it won’t break Contentlayer / acorn):
...
No luck there, next we check if it's a classic zip.

![image](/static/images/n0ps-ctf-2025-writeup/1764419376521-image.png)

It is encrypted with ZipCrypto but the compression is Deflate, so a known plaintext won’t work with these files ;-;

#### Changing Focus

Since we didn't get anywhere with `Secret.zip`, let’s focus on `SignatureLog.txt`.

![image](/static/images/n0ps-ctf-2025-writeup/1764419405429-image.png)

These seem to be [ECDSA](https://en.wikipedia.org/wiki/Elliptic_Curve_Digital_Signature_Algorithm) signatures of `Approve access for ID <id>` , let’s parse them in Python and analyze them:
...
Bamm, the `SignatureLog.txt` contains multiple signatures with same `r` meaning `nonce (k)` is reused. We can use this for a [nonce reuse attack](https://billatnapier.medium.com/ecdsa-signatures-are-great-but-be-carefull-of-nonce-reuse-eeba25d20ab7).
...
Researching online for side-channel SPA attack on ECC, we come across readings about how double-add and double-only operations differ in power traces, one such example trace is:

![image](/static/images/n0ps-ctf-2025-writeup/1764419529099-image.png)

Comparing it to our traces plot, we can clearly see a pattern of shorter gaps between peaks (double-only) and longer gaps (double-add):
...
After checking the binary output a bit, we see that the bits are correct but it is reversed so we reverse the binary output before decryption:
